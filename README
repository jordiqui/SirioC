...
<p align="center">
  <img src="docs/sirioc-logo.png" alt="SirioC chess engine logo: stellar scientific emblem with chess knight and orbital geometry" width="280" />
</p>

<p align="center">
  <strong>SirioC</strong> is an original C++20 UCI chess engine by <strong>Jorge Ruiz Centelles</strong>.
</p>

<p align="center">
  <em>Bitboards · UCI · Alpha-beta search · Syzygy · Lazy SMP · Forensic NNUE roadmap · MIT licence</em>
</p>
...
- Static evaluation with handcrafted chess terms.
...
- Draw detection for fifty-move rule, repetition and insufficient material.
- UCI interface for GUI integration.
- Unit tests and reproducible benchmarks.

The current development line is also undergoing a **forensic refactor** so future changes can be audited and merged safely. The first roadmap steps separate search parameters and history state from the large search implementation before deeper selectivity and NNUE work are attempted.
...
```mermaid
flowchart LR
    A[Board / FEN / Movegen] --> B[Search]
    B --> C[Evaluation]
    B --> D[Transposition Table]
    B --> E[Time Manager]
    C --> F[Classical Evaluation]
    C --> G[Legacy NNUE / SirioNNUE1]
    C --> H[SirioNNUE2 Roadmap]
    B --> I[UCI Output]
    J[Syzygy / Fathom] --> B
    K[Opening Book] --> B
```
...
- Polyglot-style opening-book support where configured.
...
SirioC is not trying to become stronger through untraceable patches. The project is following a controlled roadmap:

```mermaid
flowchart TD
    P000[P0-00 Guardrails and provenance policy] --> P001[P0-01 Search parameters extraction]
    P001 --> P002[P0-02 SearchHistory extraction]
    P002 --> P002B[P0-02B Dedicated history tests]
    P002B --> P003[P0-03 SirioNNUE2 backend contract]
    P003 --> P004[P0-04 SirioHalfKAv1 sparse feature encoder]
    P004 --> P005[P0-05 Python parity encoder and binary exporter]
    P005 --> P006[P0-06 Trainer and dataset pipeline]
    P006 --> P007[P0-07 Evaluation routing and gauntlets]
```
...
tsrc/tt.cpp / src/tt.cpp       Transposition-table implementation
...
> Note: exact file names may evolve as the forensic refactor progresses. Follow the migration logs under `docs/sirioc_reckless_migration/` for the authoritative development record.
...
The Arena/Fritz/Cute Chess configuration dialog exposes several UCI options. Exact availability depends on the current build.
...
- [`docs/review_checklists.md`](docs/review_checklists.md);
- [`docs/sirioc_reckless_migration/PROVENANCE_POLICY.md`](docs/sirioc_reckless_migration/PROVENANCE_POLICY.md), if present.
...
SirioC is distributed under the **MIT licence**. See [`LICENSE`](LICENSE) for details.
