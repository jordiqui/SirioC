...
# P0-92 ProbCut Helper Foundation (Disabled / No-op)

- Added ProbCut helper foundation in centralized search parameters (constants + helper contract).
...
# P0-93 ProbCut disabled main-negamax probe wiring

- ProbCut disabled main-negamax probe wiring added.
- No NNUE runtime behavior changed.
- No evaluation backend topology changed.
- No strength/Elo claim.
...
- Added deterministic ProbCut candidate classification helper with disabled/no-op behavior.
- No NNUE runtime behavior changed.
- No evaluation backend topology changed.
- No strength/Elo claim.
...
- Wired `should_cutoff_probcut(...)` in guarded main-negamax ProbCut block only, using empty reduced-result context.
- Decision is guarded by `probcut_result.has_result` and discarded locally (`(void)probcut_cutoff`).
- No return/cutoff/search/reduced-depth search behavior added.
...
- Added guarded future ProbCut return scaffold under `if (probcut_cutoff)` in main negamax.
- The guarded block preserves P0-108 observability via `record_probcut_cutoff_decision()` and now includes `return probcut_result.value;`.
- Return remains unreachable under current defaults: `selectivity_probcut_enabled = false`, guarded runtime probe path is false, and runtime reduced-search result remains empty/no-result (`has_result = false`).
...
- Added a pure ProbCut candidate eligibility helper with empty/no-candidate runtime behavior preserved.
- No NNUE runtime behavior changed.
- No evaluation backend topology changed.
- No strength/Elo claim.
...
- Added ProbCut ineligible-candidate observability with empty/no-candidate runtime behavior.
- No NNUE runtime behavior changed.
- No evaluation backend topology changed.
- No strength/Elo claim.
...
- ProbCut empty candidate-flags observability added with empty/no-candidate runtime behaviour.
- No NNUE runtime behaviour changed.
- No evaluation backend topology changed.
- No strength/Elo claim.
...
# P0-125 ProbCut runtime candidate-flags non-empty probe (no-op)
- ProbCut candidate-flags non-empty probe added with empty/no-candidate runtime behaviour.
- No NNUE runtime behaviour changed.
- No evaluation backend topology changed.
- No strength/Elo claim.
...
# P0-127 ProbCut Runtime Explicit False Candidate-Flags Builder / Empty Runtime Flags Contract

- ProbCut runtime candidate flags now use explicit false builder with empty/no-candidate runtime behaviour.
- No NNUE runtime behaviour changed.
- No evaluation backend topology changed.
- No strength/Elo claim.
...
# P0-133 ProbCut non-empty reduced-search result observability (no activation)

- Added observability for the future non-empty ProbCut reduced-search result path.
- Runtime ProbCut reduced-search result remains empty/no-result under current placeholders.
- No NNUE runtime behavior changed.
- No evaluation backend topology changed.
- No strength/Elo claim.
