...
def get_path():
    return os.path.abspath(os.path.join(CWD, args.sirioc_path))
...
class TestCLI(metaclass=OrderedClassMembers):
    ...
    def test_eval(self):
        self.engine = SirioC("eval".split(" "), True)
        assert self.engine.process.returncode == 0

    def test_go_nodes_1000(self):
        self.engine = SirioC("go nodes 1000".split(" "), True)
        assert self.engine.process.returncode == 0

    def test_go_depth_10(self):
        self.engine = SirioC("go depth 10".split(" "), True)
        assert self.engine.process.returncode == 0

    def test_go_perft_4(self):
        self.engine = SirioC("go perft 4".split(" "), True)
        assert self.engine.process.returncode == 0

    def test_go_movetime_1000(self):
        self.engine = SirioC("go movetime 1000".split(" "), True)
        assert self.engine.process.returncode == 0
    ...
    def test_go_movetime_200(self):
        self.engine = SirioC("go movetime 200".split(" "), True)
        assert self.engine.process.returncode == 0
    ...
    def test_d(self):
        self.engine = SirioC("d".split(" "), True)
        assert self.engine.process.returncode == 0

    def test_compiler(self):
        self.engine = SirioC("compiler".split(" "), True)
        assert self.engine.process.returncode == 0

    def test_license(self):
        self.engine = SirioC("license".split(" "), True)
        assert self.engine.process.returncode == 0

    def test_uci(self):
        self.engine = SirioC("uci".split(" "), True)
        assert self.engine.process.returncode == 0
...
class TestInteractive(metaclass=OrderedClassMembers):
    ...
    def test_startup_output(self):
        self.engine.starts_with("SirioC")
    ...
    def test_verify_nnue_network(self):
        ...
        self.engine.send_command("setoption name EvalFile value verify.nnue")
        self.engine.send_command("position startpos")
        self.engine.send_command("go depth 5")
        self.engine.starts_with("bestmove")
...
class TestSyzygy(metaclass=OrderedClassMembers):
    ...
    def test_syzygy_bench(self):
        ...
        self.engine.expect("Nodes searched  :*")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run SirioC with testing options")
    ...
    parser.add_argument("sirioc_path", type=str, help="Path to SirioC binary")
