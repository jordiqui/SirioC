...
void test_make_probcut_candidate_flags_preserves_candidate_flag_only() {
    const auto flags = sirio::search_params::make_probcut_candidate_flags(true, false, false, false);
    assert(flags.has_candidate_move);
    assert(!flags.is_capture);
    assert(!flags.is_noisy);
...
void test_make_probcut_candidate_flags_preserves_capture_flag() {
    const auto flags = sirio::search_params::make_probcut_candidate_flags(true, true, false, false);
    assert(flags.has_candidate_move);
    assert(flags.is_capture);
    assert(!flags.is_noisy);
    assert(!flags.is_promotion);
...
void test_make_probcut_candidate_flags_preserves_noisy_flag() {
    const auto flags = sirio::search_params::make_probcut_candidate_flags(true, false, true, false);
    assert(flags.has_candidate_move);
    assert(!flags.is_capture);
    assert(flags.is_noisy);
    assert(!flags.is_promotion);
...
void test_make_probcut_candidate_flags_preserves_promotion_flag() {
    const auto flags = sirio::search_params::make_probcut_candidate_flags(true, false, false, true);
    assert(flags.has_candidate_move);
    assert(!flags.is_capture);
    assert(!flags.is_noisy);
    assert(flags.is_promotion);
...
void test_make_probcut_candidate_flags_preserves_all_true_values() {
    const auto flags = sirio::search_params::make_probcut_candidate_flags(true, true, true, true);
    assert(flags.has_candidate_move);
    assert(flags.is_capture);
    assert(flags.is_noisy);
    assert(flags.is_promotion);
...
void test_probcut_candidate_flags_helpers_are_deterministic() {
    const auto empty_a = sirio::search_params::empty_probcut_candidate_flags();
    const auto empty_b = sirio::search_params::empty_probcut_candidate_flags();
    assert(empty_a.has_candidate_move == empty_b.has_candidate_move);
    assert(empty_a.is_capture == empty_b.is_capture);
    assert(empty_a.is_noisy == empty_b.is_noisy);
    assert(empty_a.is_promotion == empty_b.is_promotion);

    const auto made_a = sirio::search_params::make_probcut_candidate_flags(true, false, true, false);
    const auto made_b = sirio::search_params::make_probcut_candidate_flags(true, false, true, false);
    assert(made_a.has_candidate_move == made_b.has_candidate_move);
    assert(made_a.is_capture == made_b.is_capture);
    assert(made_a.is_noisy == made_b.is_noisy);
    assert(made_a.is_promotion == made_b.is_promotion);
}

void test_probcut_candidate_selector_from_source_and_flags_helper_matches_source_selector() {
    const auto flags = sirio::search_params::make_probcut_candidate_flags(true, true, false, true);
    const auto from_source = sirio::search_params::select_probcut_candidate_context_from_source(
        sirio::search_params::ProbCutCandidateSource::ExplicitFlags, flags.has_candidate_move,
        flags.is_capture, flags.is_noisy, flags.is_promotion);
    const auto from_bridge = sirio::search_params::select_probcut_candidate_context_from_flags(
        sirio::search_params::ProbCutCandidateSource::ExplicitFlags, flags);
    assert(from_source.has_candidate_move == from_bridge.has_candidate_move);
    assert(from_source.is_capture_or_noisy == from_bridge.is_capture_or_noisy);
    assert(from_source.is_promotion == from_bridge.is_promotion);
}
...
void test_probcut_candidate_selector_from_flags_rejects_false_candidate_flags() {
    const auto selected =
        sirio::search_params::select_probcut_candidate_context_from_flags(false, false, false, false);
    assert(!selected.has_candidate_move);
    assert(!selected.is_capture_or_noisy);
...
void test_probcut_candidate_selector_from_flags_explicit_quiet_candidate_is_not_tactical() {
    const auto selected = sirio::search_params::select_probcut_candidate_context_from_flags(true, false, false, false);
    assert(selected.has_candidate_move);
    assert(!selected.is_capture_or_noisy);
    assert(!selected.is_promotion);
}

void test_probcut_candidate_selector_from_flags_explicit_capture_candidate_is_tactical() {
    const auto selected = sirio::search_params::select_probcut_candidate_context_from_flags(true, true, false, false);
    assert(selected.has_candidate_move);
    assert(selected.is_capture_or_noisy);
    assert(!selected.is_promotion);
}

void test_probcut_candidate_selector_from_flags_explicit_noisy_candidate_is_tactical() {
    const auto selected = sirio::search_params::select_probcut_candidate_context_from_flags(true, false, true, false);
    assert(selected.has_candidate_move);
    assert(selected.is_capture_or_noisy);
    assert(!selected.is_promotion);
}

void test_probcut_candidate_selector_from_flags_explicit_promotion_candidate_is_preserved() {
    const auto selected = sirio::search_params::select_probcut_candidate_context_from_flags(true, false, false, true);
    assert(selected.has_candidate_move);
    assert(!selected.is_capture_or_noisy);
    assert(selected.is_promotion);
}

void test_probcut_candidate_selector_from_flags_preserves_all_flags() {
    const auto selected = sirio::search_params::select_probcut_candidate_context_from_flags(true, true, true, true);
    assert(selected.has_candidate_move);
    assert(selected.is_capture_or_noisy);
    assert(selected.is_promotion);
}

void test_probcut_candidate_selector_from_flags_matches_classifier() {
    const bool flags[2] = {false, true};
    for (const bool has_candidate_move : flags) {
        for (const bool is_capture : flags) {
            for (const bool is_noisy : flags) {
                for (const bool is_promotion : flags) {
                    const auto selected = sirio::search_params::select_probcut_candidate_context_from_flags(
                        has_candidate_move, is_capture, is_noisy, is_promotion);
                    const auto classified = sirio::search_params::classify_probcut_candidate(
                        has_candidate_move, is_capture, is_noisy, is_promotion);
                    assert(selected.has_candidate_move == classified.has_candidate_move);
                    assert(selected.is_capture_or_noisy == classified.is_capture_or_noisy);
                    assert(selected.is_promotion == classified.is_promotion);
                }
            }
        }
    }
...
void test_probcut_reduced_search_request_builder_returns_empty_without_candidate() {
    const auto request = sirio::search_params::make_probcut_reduced_search_request_from_parameters(false, 450, 3);
    assert(!request.has_request);
    assert(request.beta == 0);
    assert(request.depth == 0);
}

void test_probcut_reduced_search_request_builder_returns_empty_for_zero_depth() {
    const auto request = sirio::search_params::make_probcut_reduced_search_request_from_parameters(true, 450, 0);
    assert(!request.has_request);
    assert(request.beta == 0);
    assert(request.depth == 0);
}

void test_probcut_reduced_search_request_builder_returns_empty_for_negative_depth() {
    const auto request = sirio::search_params::make_probcut_reduced_search_request_from_parameters(true, 450, -2);
    assert(!request.has_request);
    assert(request.beta == 0);
    assert(request.depth == 0);
}

void test_probcut_reduced_search_request_builder_returns_non_empty_for_positive_depth() {
    const auto request = sirio::search_params::make_probcut_reduced_search_request_from_parameters(true, 450, 2);
    assert(request.has_request);
    assert(request.beta == 450);
    assert(request.depth == 2);
}

void test_probcut_reduced_search_request_builder_is_deterministic() {
    const auto first =
        sirio::search_params::make_probcut_reduced_search_request_from_parameters(true, -220, 4);
    const auto second =
        sirio::search_params::make_probcut_reduced_search_request_from_parameters(true, -220, 4);
    assert(first.has_request == second.has_request);
...
void test_probcut_candidate_source_none_observability_counter_lifecycle() {
    sirio::SearchHistory history;
    assert(history.probcut_candidate_source_none_count_for_tests() == 0);
    history.record_probcut_candidate_source_none();
    assert(history.probcut_candidate_source_none_count_for_tests() == 1);
    history.clear();
    assert(history.probcut_candidate_source_none_count_for_tests() == 0);
}
void test_probcut_candidate_source_explicit_flags_observability_counter_lifecycle() {
    sirio::SearchHistory history;
    assert(history.probcut_candidate_source_explicit_flags_count_for_tests() == 0);
    history.record_probcut_candidate_source_explicit_flags();
    assert(history.probcut_candidate_source_explicit_flags_count_for_tests() == 1);
    history.clear();
    assert(history.probcut_candidate_source_explicit_flags_count_for_tests() == 0);
}
...
void test_probcut_empty_candidate_flags_observability_counter_lifecycle() {
    sirio::SearchHistory history;
    assert(history.probcut_empty_candidate_flags_count_for_tests() == 0);
    history.record_probcut_empty_candidate_flags();
    assert(history.probcut_empty_candidate_flags_count_for_tests() == 1);
    history.clear();
    assert(history.probcut_empty_candidate_flags_count_for_tests() == 0);
}
...
void test_probcut_ineligible_candidate_observability_counter_lifecycle() {
    ...
    history.record_probcut_ineligible_candidate();
    assert(history.probcut_ineligible_candidate_count_for_tests() == 1);
    history.clear();
    assert(history.probcut_ineligible_candidate_count_for_tests() == 0);
}
...
void test_probcut_cutoff_decision_observability_counter_lifecycle() {
    ...
    history.record_probcut_cutoff_decision();
    assert(history.probcut_cutoff_decision_count_for_tests() == 1);
    history.clear();
    assert(history.probcut_cutoff_decision_count_for_tests() == 0);
}
void test_probcut_empty_reduced_search_result_observability_counter_lifecycle() {
    sirio::SearchHistory history;
    assert(history.probcut_empty_reduced_search_result_count_for_tests() == 0);
    history.record_probcut_empty_reduced_search_result();
    assert(history.probcut_empty_reduced_search_result_count_for_tests() == 1);
    history.clear();
    assert(history.probcut_empty_reduced_search_result_count_for_tests() == 0);
}
void test_probcut_non_empty_reduced_search_result_observability_counter_lifecycle() {
    sirio::SearchHistory history;
    assert(history.probcut_non_empty_reduced_search_result_count_for_tests() == 0);
    history.record_probcut_non_empty_reduced_search_result();
    assert(history.probcut_non_empty_reduced_search_result_count_for_tests() == 1);
    history.clear();
    assert(history.probcut_non_empty_reduced_search_result_count_for_tests() == 0);
}
...
void test_search_main_negamax_has_probcut_disabled_probe_observability_and_guarded_return_scaffold() {
    ...
    assert(negamax_source.find("const bool probcut_runtime_candidate_is_promotion = false;") != std::string::npos);
    assert(negamax_source.find("search_params::make_probcut_candidate_flags(\n            probcut_runtime_has_candidate_move,\n            probcut_runtime_candidate_is_capture,\n            probcut_runtime_candidate_is_noisy,\n            probcut_runtime_candidate_is_promotion)") != std::string::npos);
    ...
    assert(negamax_source.find("select_probcut_candidate_context_from_flags(\n            probcut_candidate_source,\n            probcut_candidate_flags)") !=
           std::string::npos);
    assert(negamax_source.find("if (probcut_candidate_source == search_params::ProbCutCandidateSource::ExplicitFlags)") !=
           std::string::npos);
    assert(negamax_source.find("context.history.record_probcut_candidate_source_none();") == std::string::npos);
    assert(negamax_source.find("context.history.record_probcut_candidate_source_explicit_flags();") !=
           std::string::npos);
    ...
    assert(negamax_source.find("const int probcut_depth = search_params::probcut_reduced_depth(depth_left);") !=
           std::string::npos);
    assert(negamax_source.find("const auto probcut_result =\n            search_params::empty_probcut_reduced_search_result();") != std::string::npos);
    assert(negamax_source.find("if (!probcut_result.has_result)") != std::string::npos);
    assert(negamax_source.find("if (probcut_result.has_result)") != std::string::npos);
    assert(negamax_source.find("context.history.record_probcut_non_empty_reduced_search_result();") !=
           std::string::npos);
    assert(negamax_source.find("context.history.record_probcut_empty_reduced_search_result();") != std::string::npos);
    ...
    assert(negamax_source.find("context.history.record_probcut_cutoff_decision();") != std::string::npos);
    const std::size_t cutoff_guard_pos = negamax_source.find("if (probcut_cutoff)");
    assert(cutoff_guard_pos != std::string::npos);
    const std::size_t cutoff_record_pos = negamax_source.find("context.history.record_probcut_cutoff_decision();", cutoff_guard_pos);
    ...
    assert(qsearch_source.find("record_probcut_empty_reduced_search_result") == std::string::npos);
    assert(qsearch_source.find("record_probcut_non_empty_reduced_search_result") == std::string::npos);
    assert(qsearch_source.find("empty_probcut_reduced_search_request") == std::string::npos);
    assert(qsearch_source.find("make_probcut_reduced_search_request_from_parameters(") == std::string::npos);
...
void run_history_tests() {
    ...
    test_make_probcut_candidate_flags_preserves_noisy_flag();
    test_make_probcut_candidate_flags_preserves_promotion_flag();
    test_make_probcut_candidate_flags_preserves_all_true_values();
    test_probcut_candidate_flags_helpers_are_deterministic();
    test_probcut_candidate_selector_from_source_and_flags_helper_matches_source_selector();
    ...
    test_probcut_candidate_selector_from_flags_rejects_false_candidate_flags();
    test_probcut_candidate_selector_from_flags_explicit_quiet_candidate_is_not_tactical();
    test_probcut_candidate_selector_from_flags_explicit_capture_candidate_is_tactical();
    test_probcut_candidate_selector_from_flags_explicit_noisy_candidate_is_tactical();
    ...
    test_probcut_reduced_search_request_builder_returns_empty_without_candidate();
    test_probcut_reduced_search_request_builder_returns_empty_for_zero_depth();
    test_probcut_reduced_search_request_builder_returns_empty_for_negative_depth();
    test_probcut_reduced_search_request_builder_returns_non_empty_for_positive_depth();
    test_probcut_reduced_search_request_builder_is_deterministic();
    ...
    test_probcut_candidate_source_none_observability_counter_lifecycle();
    ...
    test_probcut_empty_reduced_search_result_observability_counter_lifecycle();
    test_probcut_non_empty_reduced_search_result_observability_counter_lifecycle();
