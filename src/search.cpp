...
int negamax(Board &board, int depth, int alpha, int beta, int ply, Move *best_move,
    ...
    const bool probcut_runtime_has_candidate_move = false;
    ...
    const auto probcut_candidate_flags =
        search_params::make_probcut_candidate_flags(
            ...
            probcut_runtime_candidate_is_noisy,
            probcut_runtime_candidate_is_promotion);
    const bool probcut_candidate_flags_non_empty =
        search_params::probcut_candidate_flags_are_non_empty(probcut_candidate_flags);
    if (probcut_candidate_flags_non_empty) {
        context.history.record_probcut_non_empty_candidate_flags_probe();
    }
    (void)probcut_candidate_flags_non_empty;
    if (search_params::probcut_candidate_flags_are_empty(probcut_candidate_flags)) {
    ...
    if (!probcut_candidate.has_candidate_move) {
        context.history.record_probcut_empty_candidate_context();
    }
    ...
    if (probcut_probe) {
        ...
        const auto probcut_result =
            search_params::empty_probcut_reduced_search_result();
        if (probcut_result.has_result) {
            context.history.record_probcut_non_empty_reduced_search_result();
        }
        if (!probcut_result.has_result) {
            context.history.record_probcut_empty_reduced_search_result();
        }
        ...
        if (probcut_candidate_eligible) {
            context.history.record_probcut_eligible_candidate();
        }
        if (!probcut_candidate_eligible) {
