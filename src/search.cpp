...
int negamax(Board &board, int depth, int alpha, int beta, int ply, Move *best_move,
    ...
    const auto probcut_candidate_source =
        search_params::ProbCutCandidateSource::None;
    if (probcut_candidate_source == search_params::ProbCutCandidateSource::None) {
        context.history.record_probcut_candidate_source_none();
    }
    ...
    const bool probcut_candidate_is_promotion = false;
    const auto probcut_candidate =
        search_params::select_probcut_candidate_context_from_source(
